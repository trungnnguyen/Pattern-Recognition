...
for k=101:125%min(row,column)-25:min(row,column)
%   
...
%     Imr=Ur(:,1:k)*Dr(1:k,1:k)*Vr1(:,1:k)';
%     Imb=Ub(:,1:k)*Db(1:k,1:k)*Vb1(:,1:k)';
%     Img=Ug(:,1:k)*Dg(1:k,1:k)*Vg1(:,1:k)';
%     
...
%imshow(uint8(Im));
plot(1:25,error(101:125));%min(row,column)-25:min(row,column)-1));
